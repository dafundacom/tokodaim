...
interface TranslatePageFormProps {
  pageTranslationId: string
  language: LanguageType
  initialPageData?: Partial<SelectPage>
}

const TranslatePageForm = (props: TranslatePageFormProps) => {
  const { pageTranslationId, language } = props
  ...
  const { mutate: translatePageAction } = api.page.translate.useMutation({
    onSuccess: () => {
      form.reset()
...
export default TranslatePageForm
