...
import { LoadingProgress, toast } from "@tokodaim/ui"
...
const MediaList: React.FunctionComponent<MediaListProps> = (props) => {
  ...
  } = api.media.panelInfinite.useInfiniteQuery(
    { limit: 10 },
    {
      staleTime: 0,
  ...
  const handleObserver = React.useCallback(
    (entries: IntersectionObserverEntry[]) => {
      ...
      if (target.isIntersecting && hasNextPage) {
        setTimeout(() => void fetchNextPage(), 2)
      }
    },
              ...
              media.medias.map((media) => {
                return (
                  <div key={i}>
                    ...
                    33vw"
                      className="border-muted/30 bg-muted/30 !relative aspect-[1/1] h-[500px] max-w-[unset] cursor-pointer rounded-sm border-2 object-cover"
                      onClick={(
